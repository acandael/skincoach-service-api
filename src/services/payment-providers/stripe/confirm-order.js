module.exports = async function confirmOrder({
  ...
  if (existingOrder) {
    console.log(`Order already exists for payment intent ${paymentIntentId}: ${existingOrder.id}`);
    return {
      success: true,
  ...
  try {
    ...
    console.error('Failed to build Crystallize order model:', error);
  ...
  try {
    ...
    console.log(`Order created in Crystallize: ${order.id} for payment intent: ${paymentIntentId}`);
  } catch (error) {
    console.error('Failed to create order in Crystallize:', error);
  ...
  try {
    ...
    console.log(`Order ${order.id} persisted successfully`);
  } catch (error) {
    // Order was created but persistence check timed out
    // This is not critical - the order exists, just eventual consistency delay
    console.warn(`Order persistence check timed out for ${order.id}, but order was created:`, error);
